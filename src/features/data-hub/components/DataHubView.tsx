...
export function DataHubView({ customers, opportunities, workRecords, revenueRecords, followUps, profile, proposals }: Props) {
  ...
  const issues: Issue[] = [
    ...
    ...(orphanJobs.length > 0 ? [{ id: "orphan-jobs", dot: "neutral" as const, title: `${orphanJobs.length} ${jobPlural.toLowerCase()} not linked to a ${customerSingular.toLowerCase()}`, meta: `${jobPlural} · Breaks lifetime value and history`, count: orphanJobs.length, href: "/crm" }] : []),
    ...(jobMissingValue.length > 0 ? [{ id: "job-no-value", dot: "neutral" as const, title: `${jobMissingValue.length} ${jobPlural.toLowerCase()} missing job value`, meta: `${jobPlural} · Revenue reports will be incomplete`, count: jobMissingValue.length, href: "/crm" }] : []),
    ...(overdueFollowUps.length > 0 ? [{ id: "overdue-followups", dot: "danger" as const, title: `${overdueFollowUps.length} overdue ${followUpPlural.toLowerCase()}`, meta: `${followUpPlural} · These are past their due date`, count: overdueFollowUps.length, href: "/crm" }] : []),
    ...(followUpNoDueDate.length > 0 ? [{ id: "followup-no-date", dot: "neutral" as const, title: `${followUpNoDueDate.length} ${followUpPlural.toLowerCase()} with no due date`, meta: `${followUpPlural} · Hard to prioritize without a date`, count: followUpNoDueDate.length, href: "/crm" }] : []),
