...
export function DataHubView({ customers, opportunities, workRecords, revenueRecords, followUps, profile, proposals }: Props) {
  ...
  const issues: Issue[] = [
    ...(missingEmail.length > 0 ? [{ id: "missing-email", dot: "danger" as const, title: `${missingEmail.length} ${customerPlural.toLowerCase()} missing email`, meta: `${customerPlural} · Needed for outreach and follow-up`, count: missingEmail.length, href: "/customers?missing=email" }] : []),
    ...(missingPhone.length > 0 ? [{ id: "missing-phone", dot: "warning" as const, title: `${missingPhone.length} ${customerPlural.toLowerCase()} missing phone number`, meta: `${customerPlural} · Limits how quickly you can reach ${customerPlural.toLowerCase()}`, count: missingPhone.length, href: "/customers?missing=phone" }] : []),
    ...(leadMissingCustomer.length > 0 ? [{ id: "lead-no-customer", dot: "warning" as const, title: `${leadMissingCustomer.length} open ${leadPlural.toLowerCase()} not linked to a ${customerSingular.toLowerCase()}`, meta: `${leadPlural} · Breaks history and lifetime value`, count: leadMissingCustomer.length, href: "/crm?issue=lead-no-customer" }] : []),
    ...(leadMissingValue.length > 0 ? [{ id: "lead-no-value", dot: "neutral" as const, title: `${leadMissingValue.length} open ${leadPlural.toLowerCase()} missing estimated value`, meta: `${leadPlural} · Pipeline value will be understated`, count: leadMissingValue.length, href: "/crm?issue=lead-no-value" }] : []),
    ...(leadNoFollowUp.length > 0 ? [{ id: "lead-no-followup", dot: "warning" as const, title: `${leadNoFollowUp.length} open ${leadPlural.toLowerCase()} with no follow-up date`, meta: `${leadPlural} · At risk of going cold`, count: leadNoFollowUp.length, href: "/crm?issue=lead-no-followup" }] : []),
    ...(leadMissingSource.length > 0 ? [{ id: "lead-no-source", dot: "neutral" as const, title: `${leadMissingSource.length} open ${leadPlural.toLowerCase()} missing source`, meta: `${leadPlural} · Source data helps show which channels are working`, count: leadMissingSource.length, href: "/crm?issue=lead-no-source" }] : []),
    ...(orphanJobs.length > 0 ? [{ id: "orphan-jobs", dot: "neutral" as const, title: `${orphanJobs.length} ${jobPlural.toLowerCase()} not linked to a ${customerSingular.toLowerCase()}`, meta: `${jobPlural} · Breaks lifetime value and history`, count: orphanJobs.length, href: "/crm?issue=orphan-jobs" }] : []),
    ...(jobMissingValue.length > 0 ? [{ id: "job-no-value", dot: "neutral" as const, title: `${jobMissingValue.length} ${jobPlural.toLowerCase()} missing job value`, meta: `${jobPlural} · Revenue reports will be incomplete`, count: jobMissingValue.length, href: "/crm?issue=job-no-value" }] : []),
    ...(overdueFollowUps.length > 0 ? [{ id: "overdue-followups", dot: "danger" as const, title: `${overdueFollowUps.length} overdue ${followUpPlural.toLowerCase()}`, meta: `${followUpPlural} · These are past their due date`, count: overdueFollowUps.length, href: "/crm?issue=overdue-followups" }] : []),
    ...(followUpNoDueDate.length > 0 ? [{ id: "followup-no-date", dot: "neutral" as const, title: `${followUpNoDueDate.length} ${followUpPlural.toLowerCase()} with no due date`, meta: `${followUpPlural} · Hard to prioritize without a date`, count: followUpNoDueDate.length, href: "/crm?issue=followup-no-date" }] : []),
    ...(unpaidSales.length > 0 ? [{ id: "unpaid-sales", dot: "warning" as const, title: `${unpaidSales.length} ${salePlural.toLowerCase()} unpaid`, meta: `${salePlural} · Outstanding payments to collect`, count: unpaidSales.length, href: "/sales?status=pending" }] : []),
    ...(missingAddress.length > 0 ? [{ id: "missing-address", dot: "neutral" as const, title: `${missingAddress.length} ${customerPlural.toLowerCase()} missing address`, meta: `${customerPlural} · Useful for routing and service records`, count: missingAddress.length, href: "/customers?missing=address" }] : []),
  ].sort((a, b) => {
