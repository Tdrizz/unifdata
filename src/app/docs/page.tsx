        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[260px_1fr]">
            ...
            <div className="mt-3 grid gap-0.5 text-[13px]">
              {navItems.map(([label, href]) => (
                <a
                  key={href}
                  href={href}
                  className="rounded-[8px] px-3 py-2 font-medium text-ud-muted hover:bg-ud-surface-sunk hover:text-ud-ink transition-colors"
                >
                  {label}
                </a>
              ))}
          ...
          <div className="space-y-5">
            ...
            <section id="start" className="rounded-[20px] border border-ud bg-ud-surface shadow-ud p-8">
              ...
              <h2 className="mt-2 text-[24px] font-semibold tracking-[-0.02em]">Getting started</h2>
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-3">
                {[
                  ...
                  <div key={item.step} className="rounded-[14px] border border-ud bg-ud-surface-soft p-5">
                    <p className="text-[11px] font-semibold tracking-[0.12em] text-ud-faint">{item.step}</p>
                    <p className="mt-3 text-[14px] font-semibold text-ud-ink">{item.title}</p>
                    <p className="mt-2 text-[13px] leading-[1.65] text-ud-muted">{item.body}</p>
                  </div>
                ))}
              </div>

              <div className="mt-5 rounded-[12px] border border-ud bg-ud-accent-soft px-5 py-4">
                <p className="text-[13px] font-semibold text-ud-accent">Tip: Start with your customer list</p>
                <p className="mt-1 text-[13px] leading-[1.65] text-ud-muted">
                ...
                {[
                  {
                    ...
                    description: "Customers, leads, jobs, sales, and follow-ups all live in one place and connect to each other.",
                  },
                  {
                    title: "Adapt to the business type",
                    description: "Language, labels, and priorities shift based on the company's industry and operating model.",
                  },
                  {
                    title: "Turn updates into connected workflow",
                    description: "Marking an opportunity won automatically creates the linked work and revenue records.",
                  },
                ].map((item) => (
                  <div key={item.title} className="rounded-[14px] border border-ud bg-ud-surface-soft p-5">
                    <p className="text-[14px] font-semibold text-ud-ink">{item.title}</p>
                    <p className="mt-2 text-[13px] leading-[1.65] text-ud-muted">{item.description}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section id="lifecycle" className="rounded-[20px] border border-ud bg-ud-surface shadow-ud p-8">
              ...
              <div className="mt-6 grid gap-3">
                {lifecycle.map((item, index) => (
                  <div
                    key={item.title}
                    className="grid gap-4 rounded-[14px] border border-ud bg-ud-surface-soft p-5 md:grid-cols-[56px_1fr_auto] md:items-center"
                  >
                    <div className="flex h-10 w-10 items-center justify-center rounded-[10px] bg-ud-ink text-[13px] font-semibold text-white">
                      {index + 1}
                    </div>
                    <div>
                      <p className="text-[14px] font-semibold text-ud-ink">{item.title}</p>
                      <p className="mt-1 text-[13px] leading-[1.6] text-ud-muted">{item.description}</p>
                    </div>
            ...
            <section id="pages" className="rounded-[20px] border border-ud bg-ud-surface shadow-ud p-8">
              ...
              <div className="mt-6 grid gap-3 md:grid-cols-2">
                {pages.map((page) => (
                  <div key={page.path} className="rounded-[14px] border border-ud bg-ud-surface-soft p-5">
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-[14px] font-semibold text-ud-ink">{page.name}</p>
                      <div className="flex items-center gap-2">
                        <span className="rounded-full bg-ud-accent-soft px-2.5 py-0.5 text-[11px] font-semibold text-ud-accent">
                          {page.tag}
                        </span>
                        <code className="rounded-[6px] bg-ud-surface border border-ud px-2.5 py-1 text-[11px] font-semibold text-ud-muted">
                          {page.path}
                        </code>
                      </div>
                    </div>
                    <p className="mt-3 text-[13px] leading-[1.65] text-ud-muted">{page.description}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section id="industries" className="rounded-[20px] border border-ud bg-ud-surface shadow-ud p-8">
              ...
              <div className="mt-6 overflow-hidden rounded-[14px] border border-ud">
                <div className="grid grid-cols-2 border-b border-ud bg-ud-surface-sunk px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-ud-muted md:grid-cols-[200px_1fr]">
                  <span>Sector</span>
                ...
                {sectors.map((item, i) => (
                  <div
                    key={item.sector}
                    className={`grid gap-3 px-5 py-4 md:grid-cols-[200px_1fr] ${i % 2 === 0 ? "bg-ud-surface" : "bg-ud-surface-soft"}`}
                  >
                    <p className="text-[13.5px] font-semibold text-ud-ink">{item.sector}</p>
                    <p className="text-[13px] leading-[1.6] text-ud-muted">{item.wording}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section id="sync" className="rounded-[20px] border border-ud bg-ud-surface shadow-ud p-8">
              ...
              <div className="mt-6 overflow-hidden rounded-[14px] border border-ud bg-ud-ink">
                <div className="grid divide-y divide-white/10 md:grid-cols-3 md:divide-x md:divide-y-0">
                  {[
                    ...
                    <div key={item.step} className="p-6">
                      <p className="text-[11px] font-semibold tracking-[0.12em] text-white/40">{item.step}</p>
                      <p className="mt-3 text-[14px] font-semibold text-white">{item.label}</p>
                      <p className="mt-2 text-[13px] leading-[1.65] text-white/50">{item.detail}</p>
                    </div>
                  ))}
                </div>
              </div>

              <div className="mt-4 rounded-[12px] border border-ud-warning-bg bg-ud-warning-bg px-5 py-4">
                <p className="text-[13px] font-semibold text-ud-warning">Payment status is always separate</p>
                <p className="mt-1 text-[13px] leading-[1.65] text-ud-muted">
            ...
            <section id="integrations" className="rounded-[20px] border border-ud bg-ud-surface shadow-ud p-8">
              ...
              <div className="mt-6 grid gap-3 md:grid-cols-2">
                {integrations.map((integration) => (
                  <div key={integration.name} className="rounded-[14px] border border-ud bg-ud-surface-soft p-5">
                    <div className="flex items-center justify-between gap-3">
                    ...
                    <p className="mt-3 text-[13px] leading-[1.65] text-ud-muted">{integration.description}</p>
                  </div>
                ))}
              </div>

              <div className="mt-5 rounded-[14px] border border-ud bg-ud-ink p-6">
            ...
            <section id="imports" className="rounded-[20px] border border-ud bg-ud-surface shadow-ud p-8">
              ...
              <div className="mt-6 overflow-hidden rounded-[14px] border border-ud">
                <div className="grid border-b border-ud bg-ud-surface-sunk px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-ud-muted md:grid-cols-[140px_80px_1fr_160px]">
                  <span>Column</span>
                  <span>Required</span>
                ...
                {csvColumns.map((col, i) => (
                  <div
                    key={col.column}
                    className={`grid gap-2 px-5 py-4 text-[13px] md:grid-cols-[140px_80px_1fr_160px] ${i % 2 === 0 ? "bg-ud-surface" : "bg-ud-surface-soft"}`}
                  >
                    <code className="font-semibold text-ud-ink">{col.column}</code>
                    <span>
                      {col.required ? (
                        <span className="rounded-full bg-ud-ink px-2.5 py-0.5 text-[11px] font-semibold text-white">
                          Required
                        </span>
                      ) : (
                    ...
                    <span className="text-ud-muted">{col.description}</span>
              ...
              <pre className="mt-2 overflow-x-auto rounded-[14px] bg-ud-ink p-5 text-[13px] leading-[1.7] text-white/70">{`name,phone,email,address,customer_type,notes
Mike Johnson,808-555-0110,mike@example.com,Kailua-Kona HI,Residential,Interested in monthly service`}</pre>
            ...
            <section id="ai" className="rounded-[20px] border border-ud bg-ud-surface shadow-ud p-8">
              ...
              <div className="mt-5 rounded-[14px] border border-ud bg-ud-ink p-6">
                ...
                <div className="mt-4 space-y-4 text-[13px] leading-[1.7] text-white/60">
                  <div>
              ...
              <div className="mt-4 rounded-[12px] border border-ud bg-ud-accent-soft px-5 py-4">
