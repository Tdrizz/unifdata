...
/**
 * Records an SMS that has already been sent via Twilio: inserts the outbound
 * communication_messages row, updates the thread's last-message metadata, and
 * logs it on the contact's activity timeline. Shared by the reply route
 * (existing thread) and the start-conversation route (new thread) so the two
 * don't drift on what "a sent message" actually writes.
 */
